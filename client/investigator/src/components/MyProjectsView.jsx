...
function MyProjectsView({ name, email }) {
  ...
  // Fetch all projects for the investigator on component mount
  useEffect(() => {
    const fetchProjects = async () => {
      try {
        const res = await axios.get('http://localhost:8000/api/investigator/getAllProjects', {
  ...
  return (
    <Container component="main" maxWidth="md" sx={{ mt: 4 }}>
      <Paper elevation={3} sx={{ padding: 3 }}>
        <Typography variant="h4" gutterBottom>Investigator Projects</Typography>
        <TextField
          fullWidth
          margin="normal"
